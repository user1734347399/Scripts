...
Usa una descarga compatible

Si el agente ya se descargo, no lo vuelve a descargar

Si el agente no se desinstala, 
entonces no avanza con la nueva instalacion y registra un log con Errores

Primero intenta desinstalar con powershell y sino intenta con CMD

Si el agente se logra instalar, se registra otro Log de exito

Si alguien borra el ultimo agente: verifica varias cosas y vuelve a instalarlo
...
set "workDir=C:\ProgramData\Action1"
...
echo %date% %time% | Instalacion Exitosa > "%workDir%\Nuevo-Agente.txt"
"@
